...
*Setup instructions for a gitian build of DIVI using a Debian VM or physical system.*
...
Clone the git repositories for divi and gitian and then checkout the divi version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the Divicoin repository
...
To build DIVI (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the Divicoin repository.
...
```bash
    ...
    From https://github.com/Divicoin/Divicoin
...
For example:
```bash
URL=https://github.com/Divicoin/Divicoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit divi=${COMMIT} --url divi=${URL} ../Divicoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit divi=${COMMIT} --url divi=${URL} ../Divicoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit divi=${COMMIT} --url divi=${URL} ../Divicoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/divi-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/divi-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/divi-build.assert
```
...
[Divicoin/gitian.sigs](https://github.com/Divicoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
